...
void L2Project_interface(int mopt, int run,int istart, int iend,
    ...
    {  
       ...
       L2Project_interface(1,0,1-mbc,melems+mbc,node,q,aux,qI,auxI,Implicit,0.0,interf2global,global2interf,dxi,N,NI,&FluxFunc);
    }
    else
    {
#pragma omp parallel for
...
void SampleBasis_Interface(const dTensor1& spts,
                 dTensor3& phi,double xc,double dx,double xc1,double xc2,double dx1,double dx2)
{
      ...
      for (int m=1; m<=mpoints; m++)
        {
          ...
          double xi1=(xi*dx1)/2.0+xc1;
          double xi2=(xi*dx2)/2.0+xc2;
          phi.set( m,1,1, 1.0 );
          phi.set( m,2,1, sq3*(xi1-xc)/dx*2.0 );
          phi.set( m,1,2, 1.0 );
          phi.set( m,2,2, sq3*(xi2-xc)/dx*2.0 );
        }
      break;
     }
}
