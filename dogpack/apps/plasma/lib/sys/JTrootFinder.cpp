...
// All downloadable software is offered freely and without
// restriction -- although in most cases the files should be
// considered as works in progress (alpha or beta level).
//
...
double JTrootFinder::get_smallest_positive_real_root(double *op, int degree) 
{
  double zeror [MAX_DEGREE+1];
  double zeroi [MAX_DEGREE+1];
  // possibly faster way: find smallest real root
  // and if negative find smallest real root of p(-x)
  stopCode = POSITIVE_REAL_ROOT;
  desired_root = -1.;
  int numroots = rpoly(op, degree, zeror, zeroi, NULL);
  // confirm that a positive root was actually found
  assert(desired_root>0.);
  return desired_root;
}

int JTrootFinder::rpoly(double *op, int degree, double *zeror, double *zeroi, int info[] ) 
{
    assert(degree<=MAX_DEGREE);
    double t,aa,bb,cc,/* *temp (eaj commented out),*/factor,rot;
    // double *pt; // eaj commented out
    double temp[MAX_DEGREE+1]; // eaj inserted
    double pt  [MAX_DEGREE+1]; // eaj inserted
...
_40:        
    ...
    if (n == 1) {
        ...
        if( info != NULL )
           info[ degree ] = 0;
        printf("C: root #%d: %f + %f i\n",degree-1,zeror[degree-1],zeroi[degree-1]);
        switch(stopCode)
        {
          case POSITIVE_REAL_ROOT:
            if(zeroi[degree-1]==0. && zeror[degree-1]>0.)
            {
              desired_root = zeror[degree-1];
              return n+1;
            }
            else
            {
              printf("could not find positive real root.\n");
              assert(0);
            }
            break;
        }
...
/*  Calculate the final zero or pair of zeros. */
    if (n == 2) {
        ...
        printf("root #%d: %f + %f i\n",degree-2,zeror[degree-2],zeroi[degree-2]);
        printf("root #%d: %f + %f i\n",degree-1,zeror[degree-1],zeroi[degree-1]);
        switch(stopCode)
        {
          case POSITIVE_REAL_ROOT:
            if(zeroi[degree-2]==0. && zeror[degree-2]>0.)
            {
              desired_root = zeror[degree-2];
            }
            else if(zeroi[degree-1]==0. && zeror[degree-1]>0.)
            {
              desired_root = zeror[degree-1];
            }
            else
 ...
 */
            ...
            if( info != NULL )
               info[ j + 1 ] = itercnt;
            printf("A root #%d: %f + %f i\n",j,zeror[j],zeroi[j]);
            switch(stopCode)
            {
              case POSITIVE_REAL_ROOT:
                if(zeroi[j]==0. && zeror[j]>0.)
                {
                  desired_root = zeror[j];
                  return j;
                }
                break;
            }
            ...
            if (nz != 1) {
                ...
                if( info != NULL )
                  info[ j + 2 ] = 0;
            printf("B root #%d: %f + %f i\n",j+1,zeror[j+1],zeroi[j+1]);
            switch(stopCode)
            {
              case POSITIVE_REAL_ROOT:
                if(zeroi[j+1]==0. && zeror[j+1]>0.)
                {
                  desired_root = zeror[j+1];
                  return j+1;
                }
                break;
            }
 ...
 *  found.
 */
void JTrootFinder::fxshfr(int l2,int *nz)
{
 ...
 */
void JTrootFinder::quadit(double *uu,double *vv,int *nz)
{
    double ui,vi;
 ...
 */
void JTrootFinder::realit(double sss, int *nz, int *iflag)
{
    double pv,kv,t,s;
    double ms,mp,omp,ee;
    int i,j;
 ...
 */
void JTrootFinder::calcsc(int *type)
{
 ...
 *  computed in calcsc.
 */
void JTrootFinder::nextk(int *type)
{
    double temp;
        int i;
 ...
 */
void JTrootFinder::newest(int type,double *uu,double *vv)
{
 ...
 */
void JTrootFinder::quadsd(int nn,double *u,double *v,double *p,double *q,
    double *a,double *b)
{
    double c;
        int i;
 ...
 */
void JTrootFinder::quad(double a,double b1,double c,double *sr,double *si,
        double *lr,double *li)
{
        double b,d,e;
