     ...
     const dTensor2& auxvals, dTensor2& source)
{
  ...
  for (int i=1; i<=numpts; i++)
  {
    ...
    //
    source.fetch(i,_N11_i)+=q_mi_inv*2.0*(-i13*B2);
    source.fetch(i,_N12_i)+=q_mi_inv*(i13*B1-i23*B2);
    source.fetch(i,_N13_i)+=q_mi_inv*(i11*B2-i12*B1-i33*B2);
    source.fetch(i,_N22_i)+=q_mi_inv*2.0*(i23*B1);
    source.fetch(i,_N23_i)+=q_mi_inv*(i12*B2-i22*B1+i33*B1);
