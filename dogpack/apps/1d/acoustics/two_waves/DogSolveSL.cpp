        ...
        const double cflv[], string outputdir)
{
    ...
    const int mx = qnew.getsize(1); //number of elements
    ...
    dTensorBC3 aux_old(mx, maux, method[1], mbc);

    // Storage
    dTensorBC3 qstar( mx, meqn, kmax, mbc);
    dTensorBC3  wold( mx, meqn, kmax, mbc );
    dTensorBC3 wstar( mx, meqn, kmax, mbc );
    dTensorBC3  wnew( mx, meqn, kmax, mbc );
...
//          StepAdvec (dt, node, aux, smax, qold, qstar);
//          aux.set(1,1,1, -aux.get(1,1,1) );
//          
//          // Right going characteristic:
//          StepAdvec (dt, node, aux, smax, qold, qnew);
//          aux.set(1,1,1, -aux.get(1,1,1) );

//          // Combine the two solutions:
//          for( int k=0; k < qnew.numel(); k++ )
//          { qnew.vset( k, 0.5*(qnew.vget(k) + qstar.vget(k) ) ); }

            // Project onto eigenvectors
            for( int i=1-mbc; i <= mx+mbc; i++ )
            for( int k=1; k <= kmax; k++ )
            { 
                wold.set(i,1,k, 0.5*( qold.get(i,2,k) - qold.get(i,1,k) ) );
                wold.set(i,2,k, 0.5*( qold.get(i,2,k) + qold.get(i,1,k) ) );
            }

            // Left going characteristic:
            aux.set(1,1,1, -1.0 );
            StepAdvec (dt, node, aux, smax, wold, wstar);
            
            // Right going characteristic:
            aux.set(1,1,1,  1.0 );
            StepAdvec (dt, node, aux, smax, wold, wnew);

            // Combine the two solutions:
            for( int i=1-mbc; i <= mx+mbc; i++ )
            for( int k=1; k <= kmax; k++ )
            { wnew.set( i, 1, k, wstar.get(i,1,k) ); }

            // Project onto eigenvectors
            for( int i=1-mbc; i <= mx+mbc; i++ )
            for( int k=1; k <= kmax; k++ )
            { 
                qnew.set(i,1,k, ( wnew.get(i,2,k)-wnew.get(i,1,k) ) );
                qnew.set(i,2,k, ( wnew.get(i,2,k)+wnew.get(i,1,k) ) );
            }
