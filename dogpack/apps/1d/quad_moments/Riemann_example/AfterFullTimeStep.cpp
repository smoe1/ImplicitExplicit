...
// Compute the Electric Field E(x) from q. This is saved in aux(1:melems, 2, 1:kmax)
  ...
  /*
    ...
    dTensorBC3 Evals(melems, meqn, kmax, mbc,1);
                                                            ...
                                                            dTensorBC3& Evals)
{
    ...
    dTensorBC3 fvals(mx, 2 , kmax, mbc, 1);  // right hand side function

    // phi(mx,1,kmax1d) = 'electric' field
    // phi(mx,2,kmax1d) = potential  (not unique!)
    dTensorBC3 phi(mx, 2, kmax, mbc, 1);  // Electric field and potential


    for(int i=1; i<= mx; i++) 
    { 
        fvals.set(i, 1, 1, qvals.get(i,1,1) - quadMomentParams.rho0 ); 
    }
