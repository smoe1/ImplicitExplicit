...
#include "SLState.h"
#include "VlasovParams.h"  // for rho_u0
    ...
    const dTensorBC4& q, const dTensorBC4& aux, SL_state& sl_state )
{
    ...
    // TODO - this is NOT the case for they hybrid method!
    ...
    const double rho_u0 = vlasovParams.get_avg_momentum();
    /////////////////////// Save E, Et, Ett, //////////////////////////////////
    for( int i = 1; i <= mx; i++ )
    {

        // First moment has extra stuff added to it:
        int k = 1;
        double E     =  Enow.get  ( i, 1, k );
        double Et    = -M1.get ( i, 1, k ) + rho_u0;
        double Ett   = -prod.get(i,1,k) + 2.0*gradKE.get(i,1,k);
