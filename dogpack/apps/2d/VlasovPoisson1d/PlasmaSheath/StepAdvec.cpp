        ...
        for(int me=1; me<= meqn; me++)
        {
            ...
            for(int i=1; i<=mx; i++)
            {
                ...
                for( int n=1; n<= mpoints1d; n++)
                {
                    ...
                    // periodic boundary conditions hard coded here:
                    //io = iMod((io-1),mx) + 1;	


                    // Hard Coded - zero inflow boundary conditions require these if statements
                    for(int k= 1; k<=kmax; k++)
                    {
                        ...
                        else if( io == 1 )  // left cell is outside of domain, right cell is inside
                        {
                            qright.set(n,k,qold.get(io,j,me,k));
                            qleft.set(n,k,0.);
                        }
                        else if( io == (mx+1) ) // right cell is oustide of domain, left cell is inside
                        {
                            qleft.set(n,k,qold.get(mx,j,me,k));
                            qright.set(n,k,0.);
                        }
                        else // completeley outside the domain (i.e. qleft and qright point to ghost cells)
                        { 
                            qright.set(n,k,0.); 
                            qleft.set(n,k,0.);
                        }
