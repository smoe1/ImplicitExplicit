...
void L2Project_extra(
    ...
    const int jend,
    const int QuadOrder, 
    const int BasisOrder_fout,  // not used! (Why? DS)
    dTensorBC4* fout,
    void (*Func)(const dTensor2&,dTensor2&,void* data),
    void* data);
...
//      ConvertQ2dToQ1d(1, 1, mx, *ExactE, *ExactE1d);
        ...
        // Extra Source terms:
        void ExtraSource(const dTensor2& xpts, dTensor2& e, void* data);
        L2Project_extra(1, mx, 1, 1, 20, -1, ExtraE, 
                        &ExtraSource, (void*)t_ptr );
