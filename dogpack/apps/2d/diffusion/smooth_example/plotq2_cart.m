...
for j=1:my
  for i=1:mx
    qex(i,j) = exp(-8*pi*pi*time)*sin(2*pi*xc(i,j))*sin(2*pi*yc(i,j));
  end
end
...
figure(3);
clf;
pt=plot(xc(:,round(my/4+1)),qex(:,round(my/4+1)),'r-');
set(pt,'linewidth',1.5);
hold on; 
pz=plot(xc(:,round(my/4+1)),qsoln(:,round(my/4+1),1),'bo');
set(pz,'markersize',8)
set(pz,'linewidth',1);
hold off; 
axis on; box on; grid off;
