		...
		dTensorBC4* Lstar)
{
  ...
  // Specify how many and which components to diffuse
  int mdiff = 1; // how many
  iTensor1 mdiff_list(mdiff);
  mdiff_list.set(1, 1 ); // which components

  // Specify the constant viscosity for each component
  dTensor1 visc_list(mdiff);
  visc_list.set(1, 1.0 );

  // Compute gradient
  void ComputeGradient(const int mdiff, const iTensor1& mdiff_list, 
		       const dTensor1& visc_list, const dTensorBC4* q, 
		       dTensorBC4& V, dTensorBC4& W);
  ComputeGradient(mdiff,mdiff_list,visc_list,q,V,W);

  // Add divergence of gradient to Lstar
  void ComputeRHS(const int mdiff, const iTensor1& mdiff_list,
		  const dTensorBC4& V, const dTensorBC4& W, 
		  dTensorBC4* Lstar);
  ComputeRHS(mdiff,mdiff_list,V,W,Lstar);
}
		     ...
		     dTensorBC4& V, dTensorBC4& W)
{
  ...
  for (int i=(2-mbc); i<=(mx+mbc-1); i++)
    for (int j=(2-mbc); j<=(my+mbc-1); j++)
      for (int mc=1; mc<=mdiff; mc++)
	{
	  int m = mdiff_list.get(mc);
	  ...
	  for (int k=1; k<=kmax; k++)
	    {
	      V.set(i,j,m,k, visc_list.get(mc)*( FUV_p.get(k) - FUV_m.get(k) - SU.get(k) )/dx );
	      W.set(i,j,m,k, visc_list.get(mc)*( FUW_p.get(k) - FUW_m.get(k) - TU.get(k) )/dy );
	    }
...
{
  const int mx   = V.getsize(1);
  const int my   = V.getsize(2);
  const int meqn = V.getsize(3);
  const int kmax = V.getsize(4);
  const int mbc  = V.getmbc();
  const double dx = dogParamsCart2.get_dx();
  const double dy = dogParamsCart2.get_dy();
