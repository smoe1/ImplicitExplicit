...
void CreateLdiff(const int& direction, const dTensorBC2& eps, dTensorBC4& q, 
    dTensorBC4& Lstar);
void ComputeGradient_AV(const int& direction, const dTensorBC4& q, 
    dTensorBC4& V, dTensorBC4& W);
void ComputeRHS_AV(const int& direction, const dTensorBC2& eps, 
    const dTensorBC4& V, const dTensorBC4& W, dTensorBC4& Lstar);
    ...
    dTensorBC4& aux, dTensorBC4& q, dTensorBC4& Lstar)
{
    ...
    // TODO - play around with these two parameters
    double s0   = 0.00005e0*pow(double(morder),-4);
    double eps0 = 0.25;   // eps = eps0 * pow(dxymax, morder-1)
...
//              mz = mz + 1;
                ...
                // in this element
                //
                // TODO play around with this parameter!!
    ...
    mz = 1;  // TODO -- learn how to update a shared variable with openmp
    if (mz>0)
    {  
        CreateLdiff(direction, eps,q,Lstar);

//      Note: qnew = alpha1*qstar + alpha2*qnew + beta*dt*Lstar
        UpdateSoln(0.0e0, 1.0e0, 1.0e0, dt, aux, q, Lstar, q);
    }
        ...
        dTensorBC4& Lstar)
{
    ...
    // Compute gradient
    ComputeGradient_AV(direction,q,V,W);

    // Add divergence of gradient to Lstar
    ComputeRHS_AV(direction,eps,V,W,Lstar);
}
...
// -------------------------------------------------------------
void ComputeGradient_AV(const int& direction, const dTensorBC4& q, dTensorBC4& V, 
        dTensorBC4& W)
{
    int mx   = q.getsize(1);
    int my   = q.getsize(2);
...
// -------------------------------------------------------------
// Save divergence of gradient into Lstar
// -------------------------------------------------------------
void ComputeRHS_AV(const int& direction, const dTensorBC2& eps, const dTensorBC4& V, 
        ...
        {	
            ...
            for (int k=1; k<=kmax; k++)
            {
                switch( direction )
                {
                    ...
                    default:  // NO OPERATOR SPLIT HERE 
                        fprintf(stderr, "this line should not be called\n");
                        exit(1);
