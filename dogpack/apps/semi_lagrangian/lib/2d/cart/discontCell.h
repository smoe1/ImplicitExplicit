...
public:
    ...
    void set_limited_xpts(dTensor1* xc_vec, dTensor1* dx_vec, dTensor2* xpts);
    // POST: return quadrature points used for the limiter - this is a different
    // set of points than are used for the projection.
    ...
    void integrate_psi_in_time(const dTensor3& psi, dTensor2& psi_new);
	// POST: integrates out time for psi(1:mpts, 1:meqn, tpts )
	
	void project_source_w_time(const dTensor3& psi, dTensor2& psi_new);
	// POST: returns weights for the projection of source term.

    void project_source_w_time(const dTensor2& psi, dTensor2& psi_new);
...
private:
    ...
    // private methods:
    void   set1DGaussPoints();
    void   setPhi();
    double evalPhi(const int&, const double&, const double&); 
    void   evalPhi(const dTensor2&, dTensor2& );
    void   setqValues();
