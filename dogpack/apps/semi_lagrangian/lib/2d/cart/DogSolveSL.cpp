        ...
        dTensor1* dt_stages )
{
	    ...
	    else if( morder == 6)
    {
		...
		//Added by Pierson Guthrey 5/21/2015

             dTensor1 b_times( 11 );
             dTensor1 a_times( 12 );
			 double sum
		
			b_times.set(1 ,0.0414649985182624)
			b_times.set(2 ,0.198128671918067)
			b_times.set(3 ,−0.0400061921041533)
			b_times.set(4 ,0.0752539843015807)
			b_times.set(5 ,−0.0115113874206879)
			
			sum = b_times.get(1)+b_times.get(2)+b_times.get(3)+b_times.get(4)+b_times.get(5)
			
			b_times.set(6 ,0.5-sum)
			b_times.set(7 ,b_times.get(5))
			b_times.set(8 ,b_times.get(4))
			b_times.set(9 ,b_times.get(3))
			b_times.set(10,b_times.get(2))
			b_times.set(11,b_times.get(1))
		
			a_times.set(1 ,0.123229775946271)
			a_times.set(2 ,0.290553797799558)
			a_times.set(3 ,−0.127049212625417)
			a_times.set(4 ,−0.246331761062075)
			a_times.set(5 ,0.357208872795928)
			
			sum = a_times.get(1)+a_times.get(2)+a_times.get(3)+a_times.get(4)+a_times.get(5)
			
			a_times.set(6 ,1.0-2.0*sum)
			a_times.set(7 ,a_times.get(6))
			a_times.set(8 ,a_times.get(5))
			a_times.set(9 ,a_times.get(4))
			a_times.set(10,a_times.get(3))
			a_times.set(11,a_times.get(2))
			a_times.set(12,a_times.get(1))
			
			// Stage delta times
			
			dt_stages->set(1 , a_times.get(1) * dt);
			dt_stages->set(2 , b_times.get(1) * dt);
			dt_stages->set(3 , a_times.get(2) * dt);
			dt_stages->set(4 , b_times.get(2) * dt);
			dt_stages->set(5 , a_times.get(3) * dt);
			dt_stages->set(6 , b_times.get(3) * dt);
			dt_stages->set(7 , a_times.get(4) * dt);
			dt_stages->set(8 , b_times.get(4) * dt);
			dt_stages->set(9 , a_times.get(5) * dt);
			dt_stages->set(10, b_times.get(5) * dt);
			dt_stages->set(11, a_times.get(6) * dt);
			dt_stages->set(12, b_times.get(6) * dt);
			dt_stages->set(13, a_times.get(7) * dt);
			dt_stages->set(14, b_times.get(7) * dt);
			dt_stages->set(15, a_times.get(8) * dt);
			dt_stages->set(16, b_times.get(8) * dt);
			dt_stages->set(17, a_times.get(9) * dt);
			dt_stages->set(18, b_times.get(9) * dt);
			dt_stages->set(19, a_times.get(10) * dt);
			dt_stages->set(20, b_times.get(10) * dt);
			dt_stages->set(21, a_times.get(11) * dt);
			dt_stages->set(22, b_times.get(11) * dt);
			dt_stages->set(23, a_times.get(12) * dt);
			
			// Stages 1-22
              sl_state.tn = t1 = t2 = tn;
			  int stage=0
			  for (int stagepair = 1 ; stagepair <=11 ; stagepair++ ){
				  
				stage=stage+1
				sl_state.split_time->set(stage,1,t1);
				sl_state.split_time->set(stage,2,t2);
				t1 += dt_stages->get(stage);
				
				stage=stage+1
				sl_state.split_time->set(stage,1,t1);
				sl_state.split_time->set(stage,2,t2);
				t2 += dt_stages->get(stage);
			  }
			  // Stage 23
			  	stage=stage+1
				sl_state.split_time->set(stage,1,t1);
        ...
        // http://www.sciencedirect.com/science/article/pii/S0377042701004927
        //
        // PracticalsymplecticpartitionedRunge–Kutta and Runge–Kutta–Nyströmmethods
